...
const TUI_SCHEMA_URL = "https://OntoLoop.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from OntoLoop.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const OntoLoop = await OntoLoopFiles(input)
  for (const file of OntoLoop) {
...
async function OntoLoopFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "OntoLoop"),
    ...(await Filesystem.findUp(["OntoLoop.json", "OntoLoop.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "OntoLoop"))
  }
