param(
  ...
  [string]$SessionPrefix = "pq11-foundry-rollout"
)
...
try {
  ...
  $logPath = Join-Path $runtimeDir ("pq11-foundry-acceptance-" + $stamp + ".log")
  $jsonPath = Join-Path $runtimeDir ("pq11-foundry-acceptance-" + $stamp + ".json")
  $backupPath = Join-Path $runtimeDir ("pq11-autoloop.prod.backup-" + $stamp + ".toml")
  ...
  $stages = @(
    [pscustomobject]@{ name = "off"; mode = "shadow"; ratio = 0.0; session = $SessionPrefix + "-off" },
  ...
  Write-Output ("PQ11_SKILL_FOUNDRY_ACCEPTANCE_OK log=" + $logPath)
  Write-Output ("PQ11_SKILL_FOUNDRY_ACCEPTANCE_JSON=" + $jsonPath)
}
finally {
